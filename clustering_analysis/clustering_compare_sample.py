...
# this transformation will give you time since zero IN MINUTES.  Needed for ST OPTICS or clustering_analysis.
...
clusterer = hdbscan.HDBSCAN(algorithm='best', alpha=1.0, approx_min_span_tree=True,
    ...
    metric='euclidean', min_cluster_size=5, min_samples=50, p=None)
...
# # %%
# tick = datetime.datetime.now()
# from sklearn.cluster import clustering_analysis
#
# # format data for dbscan
# X = (np.radians(df.loc[:, ['lon', 'lat']].values))
# x_id = df.loc[:, 'id'].values
# # execute sklearn's OPTICS
# # 5km in radians is max eps
# dbscan = clustering_analysis(min_samples=200, eps=2 / 6371.0088, metric='euclidean',
#                 algorithm='kd_tree', n_jobs=1)
# dbscan.fit(X)
