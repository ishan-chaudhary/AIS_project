...
# Create "uid_position" table in the  database.
c = conn.cursor()
c.execute("""CREATE TABLE IF NOT EXISTS uid_posits_status
(uid text, 
time timestamp, 
sog numeric,
cog numeric,
...
def parse_SQL(file_name, conn=conn):
    ...
    c.execute("""INSERT INTO uid_posits_status (uid, time, sog, cog, heading, status)
                SELECT uid, time, sog, cog, heading, status
                FROM imported_data;""")
    conn.commit()
    c.close()
