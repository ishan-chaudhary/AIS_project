...
def pooled_clustering(uid, eps_km, min_samp, eps_time, method, print_verbose=True):
    ...
    try:
        ...
        df_results = gnact.clust.get_clusters(df_posits, eps_km=eps_km, min_samp=min_samp, time_window=eps_time,
                                              method=method)
        # write the results to the db
        if type(df_results) == type(pd.DataFrame()):
            df_results[['id', 'clust_id']].to_sql(name=table_name, con=engine_pg, if_exists='append', method='multi',
                                                  index=False)
    except Exception as e:
        print(f'UID {uid[0]} error in clustering or writing results to db.')
        print(e)
    if print_verbose:
        print(f'UID {uid[0]} complete in ', datetime.datetime.now() - start)
        uids_completed = gnact.utils.add_to_uid_tracker(uid, conn_pg)
        percentage = (uids_completed / len(uid_list)) * 100
        print(
            f'Approximately {round(percentage, 3)} complete for {eps_km} km eps, {eps_time} for time, '
    ...
    for min_samp in min_samples:
        for eps_time in epsilons_time:
            ...
            if method in ['dbscan']:
                ...
                print("Method ,ust be one of 'dbscan', 'optics', 'hdbscan', or stdbscan'.")
                break
            print(f'Starting processing for {params_name}...')
            print('Creating connections and tables...')
            try:
            ...
            with Pool(workers) as p:
                try:
                    p.starmap(pooled_clustering,
                              zip(uid_list, repeat(eps_km), repeat(min_samp), repeat(eps_time), repeat(method)))
                except Exception as e:
            ...
            print('Cleaning column name for results...')
            try:
            ...
            try:
                ...
                print('Unable to update clustering_results table.')
                print(e)
            ...
            try:
                sql_insert = f"""INSERT INTO clustering_times (run, name, time_diff) 
                                 VALUES ({first_tick}, {params_name}, {iteration_lapse}), 
                                 ON CONFLICT (name) DO UPDATE 
                                     SET run = excluded.run, 
                                     seconds = excluded.time_diff;"""
                c.execute(sql_insert)
                conn.commit()
                print(f'Iteration logged at {datetime.datetime.now()}.')
